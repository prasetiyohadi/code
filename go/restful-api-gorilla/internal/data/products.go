...
// ErrProductNotFound is an error raised when a product can not be found in
// the database
...
// ProductsDB is a type injector of Products
type ProductsDB struct {
	currency proto.CurrencyClient
	log      hclog.Logger
	rates    map[string]float64
	client   proto.Currency_SubscribeRatesClient
}
...
// currency service.
func NewProductsDB(c proto.CurrencyClient, l hclog.Logger) *ProductsDB {
...
func (p *ProductsDB) handleUpdates() {
	...
	for {
		...
		// this is normally how terminal request a reconnect
		if err != nil {
		...
		if ge := srr.GetError(); ge != nil {
			...
			if sre.Code() == codes.InvalidArgument {
				...
				if d := sre.Details(); len(d) > 0 {
					...
					if rr, ok := d[0].(*proto.RateRequest); ok {
		...
		if rr := srr.GetRateResponse(); rr != nil {
			p.log.Info("Received updated rate from the server", "dest", rr.GetDestination().String())
...
func (p *ProductsDB) getRate(destination string) (float64, error) {
	...
	if err != nil {
		...
		// if this is an InvalidArgument exception sanitize the message before returning
